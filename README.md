...
| Thời gian trích xuất / chứng từ                  | **p50 26s** trên benchmark có kiểm soát ([`metrics/`](metrics/)) — nhiều file chạy song song nên cả bộ ≈ file chậm nhất. Dashboard hiển thị p50 trên **toàn bộ** dữ liệu vận hành (hiện ~40s, gồm cả các lượt model chập chờn phải retry) — chúng em công bố cả hai, không giấu số xấu |
...
- **Chống prompt injection từ chứng từ**: nội dung tài liệu được xử lý như dữ liệu KHÔNG tin cậy — model chỉ được trả JSON theo schema đóng (`response_mime_type: application/json`), không có tool-use nào kích hoạt được từ văn bản trong tài liệu, và mọi giá trị đều đi qua rule đối chiếu + người phê duyệt trước khi xuất core-banking. Một chứng từ chứa chỉ thị độc hại tệ nhất chỉ tạo ra một field sai — và field sai thì bị chính cross-check + review queue chặn lại
...
| --- | --- |
| Chất lượng triển khai kỹ thuật (20đ) | Live production + [số liệu đo thật](#số-liệu-đo-thật-không-phải-ước-lượng) có script tái lập ([`metrics/`](metrics/)), xử lý song song, cache 2 lớp, unit test cho lõi chuẩn hóa + đối chiếu ([`tests/`](tests/)) chạy CI GitHub Actions |
